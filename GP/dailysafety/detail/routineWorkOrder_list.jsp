...
<div class="table-responsive">
    <table class="table table-sm" aria-label="경상오더-조회결과">
        <thead>
        <%--<tr>
            <th scope="col" name="excelCol" data-field="첨부" class="sort icon-up">첨부</th>
            <th scope="col" name="excelCol" data-field="RBM검사" class="sort icon-up">RBM검사</th>
            ...
            <th scope="col" name="excelCol" data-field="작업요청번호" class="sort icon-up">작업요청번호</th>
            ...
            <th scope="col" name="excelCol" data-field="중요도" class="sort icon-up">중요도</th>
            ...
            <th scope="col" name="excelCol" data-field="작업시간" class="sort icon-up">작업시간</th>
        </tr>--%>
        <tr>
            <th scope="col" name="excelCol" data-field="첨부">첨부</th>
            <th scope="col" name="excelCol" data-field="RBM검사">RBM검사</th>
            ...
            <th scope="col" name="excelCol" data-field="작업요청번호">작업요청번호</th>
            ...
            <th scope="col" name="excelCol" data-field="중요도">중요도</th>
            ...
            <th scope="col" name="excelCol" data-field="작업시간">작업시간</th>
        </tr>
        </thead>
        <tbody>
        <%-- 데이터가 없을 경우 --%>
        ...
        <c:forEach var="data" items="${list}" varStatus="status">
            <tr class="_TR_RESULT_DATA" data-no="${data.woNo}" onclick="fnRoutineWorkOrderResultDetailView($(this))">
                <c:choose>
                ...
                <c:choose>
                    <c:when test="${data.isImg == 'Y'}">
                        <td data-field="RBM 검사">
                            <!-- 체크유무 -->
                            <span class="yn yes"> Y </span>
                        </td>
                    </c:when>
                    <c:when test="${data.isImg != 'Y'}">
                        <td data-field="RBM 검사">
                            <!-- 체크유무 : N인 경우-->
                            <span class="yn no"> N </span>
                        </td>
                    </c:when>
                </c:choose>
                ...
                <td data-field="작업요청번호">${data.noticeNo}</td>
                <td data-field="오더명">${data.description}</td>
                <td data-field="설계일">${data.requestDate}</td>
                <td data-field="감독자">${data.planbyName}</td>
                ...
                <td data-field="실적경비"><fmt:formatNumber value="${data.planExpenseCost}" type="number"/></td>
                <td data-field="실적금액계"><fmt:formatNumber value="${data.resultPartCost + data.resultCraftCost + data.planExpenseCost}" type="number"/></td>
                <td data-field="설비번호">${data.itemNo}</td>
                <td data-field="설비명">${data.itemDesc}</td>
                <td data-field="작업시간">${data.workTime}</td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
