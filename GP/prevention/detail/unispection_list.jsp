    ...
    <div>
        ...
        <div class="btn-box">
            ...
            <c:if test="${sessionScope.loginInfo.iui_isadmin ne '999'}">
                <button type="button" class="btn btn-primary btn-file-download" onclick="reportExcelDownload6()">엑셀 다운로드</button>
            </c:if>
        </div>
    </div>
</div>
...
<div class="table-responsive">
    <table class="table table-sm" id="tblInspectorResultList" aria-label="미점검-일정-리스트">
        <thead>
        <tr>
            <th scope="col" name="excelCol" data-field="점검계획일자">점검계획일자</th>
            <th scope="col" name="excelCol" data-field="설비번호">설비번호</th>
            <th scope="col" name="excelCol" data-field="설비명">설비명</th>
            <th scope="col" name="excelCol" data-field="점검주기">점검주기</th>
        ...
        <c:forEach var="data" items="${list}" varStatus="status">
            <tr>
                <td data-field="점검계획일자"><c:out value='${fn:substring(data.actualDate, 0, 4)}-'/><c:out value='${fn:substring(data.actualDate, 4, 6)}-'/><c:out value='${fn:substring(data.actualDate, 6, 8)}'/></td>
                <td data-field="설비번호">${data.equipNo}</td>
                <td data-field="설비명">${data.equipNm}</td>
                <td data-field="점검주기">${data.periodNm}</td>
                <td data-field="정비부서">${data.deptNm}</td>
...
<script>
    ...
    function reportExcelDownload6() {
        ...
        $.ajax({
            type: "POST"
            , url: "/prevention/unispectionListexcelDownload.do"
            ...
            , success: function (response, status, xhr) {
                ...
                link.download = "미점검일정_" + formattedDate + ".xlsx";
                link.click();  // 다운로드 트리거
            }
