(function () {
	...
	function choosePlacement(hotspot) {
		...
		// 1281px 이상부터 클래스 기반 좌/우 적용
		if (hotspot.classList.contains('info-left')) {
			return 'left';
		}

		if (hotspot.classList.contains('info-right')) {
			return 'right';
		}
	...
	function setLinkedTableState(hotspot, isActive) {
		if (!hotspot) return;

		const plantId = hotspot.dataset.plant;
		if (!plantId) return;

		const tables = document.querySelectorAll(
			'.map-glass-table[data-plant="' + plantId + '"]'
		);

		tables.forEach(function (table) {
			table.classList.toggle('is-hover', isActive);
		});
	}

	function restartLinkedTableEffect(hotspot) {
		if (!hotspot) return;
		...
		tables.forEach(function (table) {
			table.classList.remove('is-hover');

			void table.offsetWidth;

			table.classList.add('is-hover');
		});
	}
	...
	function closeHotspot(hotspot) {
		...
		setLinkedTableState(hotspot, false);
	...
	function positionInfoBox(hotspot) {
		...
		const direction = choosePlacement(hotspot);
	...
	hotspots.forEach(function (hotspot) {
		...
		pin.addEventListener('click', function (e) {
			...
			restartLinkedTableEffect(hotspot);
			openHotspot(hotspot);
		});
