...
var _modelType;
var _modelId;
var wpBridge = window.wpBridge;
var wpDataPreloader = window.wpDataPreloader;
var unityObj = null;
...
function openModelInfoPopup(modelId) {
    ...
    /* Open winbox url : 멀티뷰 연결 */
    fnOpenPopupStandard("/multiview/index.do?t=M&modelId=" + modelId, "3D Model > 설비상세정보");
}
 ...
 */
function openPanoInfoPopup(panoLv, panoId, type) {
	...
	if(type != "PANO"){
		$.ajax({
		    type: "post",
		    url: "/multiview/modelIegNo.do",
		    data: { modelId: panoId },
		    success: function(iegNo) {
				loadPanorama(iegNo);
		    }
		});
	}else{
	        ...
	        $.ajax({
	            ...
	            success: function (data) {
	                if (data !== "") {
	                    ...
	                    // load panorama
	                    fnOpenPopupStandard("/pcm/vi/panoview.do?pct_sn=84&tagno=" + data.pIdx, "파노라마");
	                } else {
	                    alert("파노라마 정보가 없습니다.");
	                }
	            },
...
/**
 * 설비번호 기준 파노라마 정보 조회 후 파노라마 팝업 호출
 * @param iegNo 설비번호
 */
function loadPanorama(iegNo){
    var tagNumberJason = {};
    var popTargetInfo = $('<input type="hidden" id="_openPanoramaPopup" data-title="파노라마"/>');
...
function initializeOnLoad() {
    ...
    if (this.wpBridge == null && window.wpBridge != null)
        this.wpBridge = window.wpBridge;
...
function fnSetPreloading3dModel() {
    if (this.wpDataPreloader == null && window.wpDataPreloader != null)
        this.wpDataPreloader = window.wpDataPreloader;

    if (this.wpDataPreloader) {
        this.wpDataPreloader.setCurrentPowerPlant(eqOrgNo);
    }
    if (unityObj) {
...
function fnSetLoadIdmsPage(isIdmsPage, scale) {
    var payload = isIdmsPage + "," + scale;

    if (this.wpBridge) {
        this.wpBridge.toggleIdms(payload);
    }
    if (unityObj) {
 ...
 * @param modelId String
 */
function cctvToUnity(modelTarget, cctvId) {
    try {
        // 창닫기 처리 부분
        window.parent.$(".winbox:not(.min) .wb-min").trigger('click');

        var systemTarget = modelTarget + "," + cctvId;

        if (this.wpBridge) {
            this.wpBridge.loadModelFromWeb(systemTarget);
        }
...
function modelLoadToUnity(modelType, modelId) {
    try {
        if (_isIdmsPage === false) {
            fnOperationControlView(modelType);
        }
...
/**
 * Web -> 3D Viewer 렌더링 활성화 여부 전달
 * @param isRenderActive
 */
function setRenderActive(isRenderActive) {
    if (this.wpBridge) {
        this.wpBridge.setRenderActive(isRenderActive);
    }
}
